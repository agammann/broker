...
it.each(["", "/"])(
  "real stdio MCP client authenticates through gateway, Chromium, password + TOTP and retrieves only authorized invoice fields (gateway suffix %j)",
  async (suffix) => {
    ...
    const dir = mkdtempSync(resolve(tmpdir(), "broker-mcp-")),
      ...
      await client.connect(
        new StdioClientTransport({
          ...
          env: {
            ...
            BROKER_GATEWAY_URL: "http://127.0.0.1:14311" + suffix,
          },
          stderr: "pipe",
        }),
      );
